...
func HTTPReleaseSmContext(c *gin.Context) {
	...
	stats.IncrementN11MsgStats(smf_context.SMF_Self().NfInstanceID, string(svcmsgtypes.NsmfPDUSessionReleaseSmContext), "In", "", "")
	...
	if err != nil {
		...
		stats.IncrementN11MsgStats(smf_context.SMF_Self().NfInstanceID, string(svcmsgtypes.NsmfPDUSessionReleaseSmContext), "Out", http.StatusText(http.StatusBadRequest), "Malformed")
		return
	}
	...
	txn := transaction.NewTransaction(req.Body.(models.ReleaseSmContextRequest), nil, svcmsgtypes.SmfMsgType(svcmsgtypes.NsmfPDUSessionReleaseSmContext))
	...
	stats.IncrementN11MsgStats(smf_context.SMF_Self().NfInstanceID, string(svcmsgtypes.NsmfPDUSessionReleaseSmContext), "Out", http.StatusText(http.StatusNoContent), "")
...
func HTTPUpdateSmContext(c *gin.Context) {
	...
	stats.IncrementN11MsgStats(smf_context.SMF_Self().NfInstanceID, string(svcmsgtypes.NsmfPDUSessionUpdateSmContext), "In", "", "")
	...
	if err != nil {
		...
		stats.IncrementN11MsgStats(smf_context.SMF_Self().NfInstanceID, string(svcmsgtypes.NsmfPDUSessionUpdateSmContext), "Out", http.StatusText(http.StatusBadRequest), "Malformed")
		log.Print(err)
		return
	}
	...
	txn := transaction.NewTransaction(req.Body.(models.UpdateSmContextRequest), nil, svcmsgtypes.SmfMsgType(svcmsgtypes.NsmfPDUSessionUpdateSmContext))
	txn.CtxtKey = smContextRef
	go txn.StartTxnLifeCycle(fsm.SmfTxnFsmHandle)
	<-txn.Status
	...
	stats.IncrementN11MsgStats(smf_context.SMF_Self().NfInstanceID, string(svcmsgtypes.NsmfPDUSessionUpdateSmContext), "Out", http.StatusText(HTTPResponse.Status), "")
