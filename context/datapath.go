...
import (
	...
	"github.com/omec-project/util_3gpp" //nolint:gci
)
...
func (node *DataPathNode) ActivateUpLinkTunnel(smContext *SMContext) error {
	...
	if pccRuleUpdate != nil {
		...
		for name, rule := range addRules {
			if pdr, err := destUPF.BuildCreatePdrFromPccRule(rule); err == nil { //nolint:govet
		...
		// Default PDR
		if pdr, err := destUPF.AddPDR(); err != nil { //nolint:govet
...
func (node *DataPathNode) DeactivateDownLinkTunnel(smContext *SMContext) {
	...
	teid := node.DownLinkTunnel.TEID
	smfContext.DrsmCtxts.TeidPool.ReleaseInt32ID(int32(teid)) //nolint:errcheck
	node.DownLinkTunnel = &GTPTunnel{}
}
...
func (dataPath *DataPath) validateDataPathUpfStatus() error {
	...
	for curDataPathNode := firstDPNode; curDataPathNode != nil; curDataPathNode = curDataPathNode.Next() {
		logger.PduSessLog.Infof(
			"Nodes in Data Path [%v] and status [%v]",
			curDataPathNode.UPF.NodeID.ResolveNodeIdToIp().
				String(),
			curDataPathNode.UPF.UPFStatus.String(),
		)
...
func (dpNode *DataPathNode) CreatePccRuleQer(
	smContext *SMContext,
	qosData string,
	tcData string,
) (*QER, error) {
...
// ActivateUpLinkPdr
func (dpNode *DataPathNode) ActivateUpLinkPdr(
	smContext *SMContext,
	defQER *QER,
	defPrecedence uint32,
) error {
	...
	for name, ULPDR := range curULTunnel.PDR {
		...
		if nextULDest := dpNode.Next(); nextULDest != nil {
			...
			iface = nextULTunnel.DestEndPoint.UPF.GetInterface(
				models.UpInterfaceType_N9,
				smContext.Dnn,
			)
...
func (dpNode *DataPathNode) ActivateDlLinkPdr(
	smContext *SMContext,
	defQER *QER,
	defPrecedence uint32,
	dataPath *DataPath,
	...
	for name, DLPDR := range curDLTunnel.PDR {
		...
		logger.PduSessLog.Traceln(
			"Current DP Node IP: ",
			dpNode.UPF.NodeID.ResolveNodeIdToIp().String(),
		)
		...
		if nextDLDest := dpNode.Prev(); nextDLDest != nil {
			...
			if anIP := smContext.Tunnel.ANInformation.IPAddress; anIP != nil {
				...
				DLPDR := ANUPF.DownLinkTunnel.PDR["default"] //nolint:govet //TODO: Iterate over all PDRs
				DLFAR := DLPDR.FAR
		...
		if curDataPathNode.DownLinkTunnel != nil {
			if curDataPathNode.DownLinkTunnel.SrcEndPoint == nil {
				for _, DNDLPDR := range curDataPathNode.DownLinkTunnel.PDR {
					DNDLPDR.PDI.SourceInterface = pfcpType.SourceInterface{
