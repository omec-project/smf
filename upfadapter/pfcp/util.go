...
func ForwardPfcpMsgToUpf(udpPodMsg config.UdpPodPfcpMsg) ([]byte, error) {
	...
	case pfcp.PFCP_ASSOCIATION_SETUP_REQUEST:
		...
		if config.IsUpfAssociated(udpPodMsg.UpNodeID) {
			...
			pfcpRsp, _ := handler.BuildPfcpAssociationResponse(
				&udpPodMsg.UpNodeID,
				pMsg.Header.SequenceNumber,
			)
		...
		logger.AppLog.Debugf(
			"association request, existing node id[%v], replaced by [%v]",
			s.NodeID,
			config.UpfAdapterIp,
		)

		// replace SMF NodeId with of upf-adapter's one
		s.NodeID = &pfcpType.NodeID{
			NodeIdType:  pfcpType.NodeIdTypeIpv4Address,
			NodeIdValue: config.UpfAdapterIp,
		}
