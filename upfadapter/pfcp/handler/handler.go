...
func HandlePfcpAssociationSetupResponse(msg *pfcpUdp.Message) {
	...
	logger.PfcpLog.Debugf(
		"HandlePfcpAssociationSetupResponse, recovery timestamp [%v] ",
		pMsgBody.RecoveryTimeStamp,
	)
...
func HandlePfcpHeartbeatResponse(msg *pfcpUdp.Message) {
	...
	logger.PfcpLog.Debugf(
		"pfcp heartbeat response recovery timestamp [%v] ",
		pMsgBody.RecoveryTimeStamp,
	)
	msg.PfcpMessage.Body = pMsgBody
	// Encode pfcp rsp to json and send to http txn
...
func BuildPfcpAssociationResponse(nodeId *pfcpType.NodeID, seqNo uint32) (*pfcp.Message, error) {
	logger.AppLog.Debugf(
		"building pfcp association response for upf [%v], seqNo [%v]",
		nodeId,
		seqNo,
	)
	if upf := config.GetUpfFromNodeId(nodeId); upf != nil {
		...
		logger.AppLog.Debugf(
			"stored association response recovery timestamp: %v",
			pfcpMsg.RecoveryTimeStamp,
		)
...
func BuildPfcpHeartBeatResponse(nodeId *pfcpType.NodeID, seqNo uint32) (*pfcp.Message, error) {
	logger.PfcpLog.Debugf(
		"building pfcp heartbeat response for upf:[%v], seqNo:[%v]",
		nodeId,
		seqNo,
	)
	if upf := config.GetUpfFromNodeId(nodeId); upf != nil {
		...
		logger.PfcpLog.Debugf(
			"stored heartbeat rsp recovery timestamp: %v",
			pfcpMsg.RecoveryTimeStamp,
		)
