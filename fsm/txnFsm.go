...
func (SmfTxnFsm) TxnLoadCtxt(txn *transaction.Transaction) (transaction.TxnEvent, error) {
	...
	case svcmsgtypes.CreateSmContext:
		...
		if smCtxtRef, err := smf_context.ResolveRef(createData.Supi, createData.PduSessionId); err == nil {
			...
			producer.HandlePduSessionContextReplacement(smCtxtRef) //nolint:errcheck
		}
		// Create fresh context
		txn.Ctxt = smf_context.NewSMContext(createData.Supi, createData.PduSessionId)
		txn.CtxtKey, _ = smf_context.ResolveRef(createData.Supi, createData.PduSessionId) //nolint:errcheck
	...
	case svcmsgtypes.N1N2MessageTransferFailureNotification:
		txn.Ctxt = smf_context.GetSMContext(txn.CtxtKey)
	default:
		txn.TxnFsmLog.Errorf(
			"handle event[%v], next-event[%v], unknown msgtype [%v] ",
			transaction.TxnEventLoadCtxt.String(),
			transaction.TxnEventFailure.String(),
			txn.MsgType,
		)
	...
	if txn.Ctxt.(*smf_context.SMContext) == nil {
		txn.TxnFsmLog.Errorf(
			"handle event[%v], ctxt [%v] not found",
			transaction.TxnEventLoadCtxt.String(),
			txn.CtxtKey,
		)
...
func (SmfTxnFsm) TxnCtxtPost(txn *transaction.Transaction) (transaction.TxnEvent, error) {
	...
	if smContext.ActiveTxn != nil {
		...
		txn.TxnFsmLog.Debugf(
			"event[%v], next-event[%v], txn queued ",
			transaction.TxnEventCtxtPost.String(),
			transaction.TxnEventExit.String(),
		)
...
func (SmfTxnFsm) TxnProcess(txn *transaction.Transaction) (transaction.TxnEvent, error) {
	...
	if smContext == nil {
		txn.TxnFsmLog.Errorf(
			"event[%v], next-event[%v], SM context invalid ",
			transaction.TxnEventProcess.String(),
			transaction.TxnEventFailure.String(),
		)
	...
	if err := HandleEvent(smContext, event, eventData); err != nil {
		smContext.SubFsmLog.Errorf(
			"handle event[%v], err [%s]",
			transaction.TxnEventProcess.String(),
			err.Error(),
		)
...
func (SmfTxnFsm) TxnSuccess(txn *transaction.Transaction) (transaction.TxnEvent, error) {
	...
	case svcmsgtypes.PfcpSessCreate:
		...
		nextTxn := transaction.NewTransaction(
			nil,
			nil,
			svcmsgtypes.SmfMsgType(svcmsgtypes.N1N2MessageTransfer), //nolint:unconvert
		)
...
func (SmfTxnFsm) TxnFailure(txn *transaction.Transaction) (transaction.TxnEvent, error) {
	...
	case svcmsgtypes.PfcpSessCreate:
		if txn.Ctxt != nil &&
			txn.Ctxt.(*smf_context.SMContext).SMContextState == context.SmStatePfcpCreatePending {
			nextTxn := transaction.NewTransaction(
				nil,
				nil,
				svcmsgtypes.SmfMsgType(svcmsgtypes.PfcpSessCreateFailure), //nolint:unconvert
			)
			nextTxn.Ctxt = txn.Ctxt
			smContext := txn.Ctxt.(*smf_context.SMContext)
			smContext.SMTxnBusLock.Lock()
	...
	case svcmsgtypes.UpdateSmContext:
		if txn.Ctxt == nil {
			logger.PduSessLog.Warnf(
				"PDUSessionSMContextUpdate, SMContext[%s] is not found",
				txn.CtxtKey,
			)
