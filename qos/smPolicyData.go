...
func BuildSmPolicyUpdate(
	smCtxtPolData *SmCtxtPolicyData,
	smPolicyDecision *models.SmPolicyDecision,
) *PolicyUpdate {
	update := &PolicyUpdate{}
	...
	// Qos Flows update
	update.QosFlowUpdate = GetQosFlowDescUpdate(
		smPolicyDecision.QosDecs,
		smCtxtPolData.SmCtxtQosData.QosData,
	)

	// Pcc Rules update
	update.PccRuleUpdate = GetPccRulesUpdate(
		smPolicyDecision.PccRules,
		smCtxtPolData.SmCtxtPccRules.PccRules,
	)

	// Session Rules update
	update.SessRuleUpdate = GetSessionRulesUpdate(
		smPolicyDecision.SessRules,
		smCtxtPolData.SmCtxtSessionRules.SessionRules,
	)

	// Traffic Control Data update
	update.TCUpdate = GetTrafficControlUpdate(
		smPolicyDecision.TraffContDecs,
		smCtxtPolData.SmCtxtTCData.TrafficControlData,
	)

	// Condition Data update
	update.CondDataUpdate = GetConditionDataUpdate(
		smPolicyDecision.Conds,
		smCtxtPolData.SmCtxtCondData.CondData,
	)
