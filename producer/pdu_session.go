...
*/
func HandlePDUSessionSMContextCreate1(eventData interface{}) error {

	txn := eventData.(*transaction.Transaction)
	req := txn.Req.(models.PostSmContextsRequest)

	rsp, errStr, _ := HandlePDUSessionSMContextCreate(req, txn.Ctxt.(*smf_context.SMContext))

	txn.Rsp = rsp

	if errStr != "" {
		txn.Err = fmt.Errorf("%v", errStr)
	}
	return nil
}
...
func HandlePDUSessionSMContextCreate(request models.PostSmContextsRequest, smContext *smf_context.SMContext) (*http_wrapper.Response, string, *smf_context.SMContext) {
	//GSM State
	...
	metrics.IncrementSvcUdmMsgStats(smf_context.SMF_Self().NfInstanceID, string(svcmsgtypes.NudmSmSubscriptionDataRetrieval), "Out", "", "")
	...
	if sessSubData, rsp, err := SubscriberDataManagementClient.
		...
		metrics.IncrementSvcUdmMsgStats(smf_context.SMF_Self().NfInstanceID, string(svcmsgtypes.NudmSmSubscriptionDataRetrieval), "In", http.StatusText(rsp.StatusCode), err.Error())
		...
		if len(sessSubData) > 0 {
			metrics.IncrementSvcUdmMsgStats(smf_context.SMF_Self().NfInstanceID, string(svcmsgtypes.NudmSmSubscriptionDataRetrieval), "In", http.StatusText(rsp.StatusCode), "")
			...
			metrics.IncrementSvcUdmMsgStats(smf_context.SMF_Self().NfInstanceID, string(svcmsgtypes.NudmSmSubscriptionDataRetrieval), "In", http.StatusText(rsp.StatusCode), "NilSubscriptionData")
	...
	metrics.IncrementSvcPcfMsgStats(smf_context.SMF_Self().NfInstanceID, string(svcmsgtypes.NpcfSmPolicyAssociationCreate), "Out", "", "")
	if smPolicyDecisionRsp, httpStatus, err := consumer.SendSMPolicyAssociationCreate(smContext); err != nil {
		metrics.IncrementSvcPcfMsgStats(smf_context.SMF_Self().NfInstanceID, string(svcmsgtypes.NpcfSmPolicyAssociationCreate), "In", http.StatusText(httpStatus), err.Error())
		...
		metrics.IncrementSvcPcfMsgStats(smf_context.SMF_Self().NfInstanceID, string(svcmsgtypes.NpcfSmPolicyAssociationCreate), "In", http.StatusText(httpStatus), "error")
...
func HandlePDUSessionSMContextUpdate1(eventData interface{}) error {

	txn := eventData.(*transaction.Transaction)
	req := txn.Req.(models.UpdateSmContextRequest)

	rsp, err := HandlePDUSessionSMContextUpdate(txn.Ctxt.(*smf_context.SMContext), req)
	txn.Rsp = rsp
	if err != nil {
		txn.Err = fmt.Errorf("%v", err.Error())
		return err
	}
...
func HandlePDUSessionSMContextUpdate(smContext *smf_context.SMContext, body models.UpdateSmContextRequest) (*http_wrapper.Response, error) {
	// GSM State
	...
	case models.N2SmInfoType_PDU_RES_REL_RSP:
		...
		if smContext.PDUSessionRelease_DUE_TO_DUP_PDU_ID {
			if smContext.SMContextState != smf_context.SmStateInActivePending {
				// Wait till the state becomes SmStateActive again
				// TODO: implement sleep wait in concurrent architecture
				smContext.SubPduSessLog.Warnf("PDUSessionSMContextUpdate, SMContext state[%v] should be SmStateActivePending",
					smContext.SMContextState.String())
			}
			smContext.ChangeState(smf_context.SmStateInit)
			smContext.SubCtxLog.Traceln("PDUSessionSMContextUpdate, SMContextState Change State: ", smContext.SMContextState.String())
...
func HandlePDUSessionSMContextRelease1(eventData interface{}) error {

	txn := eventData.(*transaction.Transaction)
	req := txn.Req.(models.ReleaseSmContextRequest)

	rsp, err := HandlePDUSessionSMContextRelease(txn.Ctxt.(*smf_context.SMContext), req)
	txn.Rsp = rsp
	if err != nil {
		txn.Err = fmt.Errorf("%v", err.Error())
		return err
...
func HandlePDUSessionSMContextRelease(smContext *smf_context.SMContext, body models.ReleaseSmContextRequest) (*http_wrapper.Response, error) {
