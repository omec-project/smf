...
func HandleUpdateN1Msg(
	...
	if body.BinaryDataN1SmMessage != nil {
		smContext.SubPduSessLog.Traceln(
			"PDUSessionSMContextUpdate, Binary Data N1 SmMessage isn't nil!",
		)
		m := nas.NewMessage()
		err := m.GsmMessageDecode(&body.BinaryDataN1SmMessage)
		smContext.SubPduSessLog.Traceln(
			"PDUSessionSMContextUpdate, Update SM Context Request N1SmMessage: ",
			m,
		)
		if err != nil {
		...
		case nas.MsgTypePDUSessionReleaseRequest:
			smContext.SubPduSessLog.Infof(
				"PDUSessionSMContextUpdate, N1 Msg PDU Session Release Request received",
			)
			if smContext.SMContextState != smf_context.SmStateActive {
				// Wait till the state becomes SmStateActive again
				// TODO: implement sleep wait in concurrent architecture
				smContext.SubPduSessLog.Infof(
					"PDUSessionSMContextUpdate, SM Context State[%v] should be SmStateActive",
			...
			response.JsonData.N1SmMsg = &models.RefToBinaryData{
		...
		case nas.MsgTypePDUSessionReleaseComplete:
			smContext.SubPduSessLog.Infof(
				"PDUSessionSMContextUpdate, N1 Msg PDU Session Release Complete received",
			)
			if smContext.SMContextState != smf_context.SmStateInActivePending {
				// Wait till the state becomes SmStateActive again
				// TODO: implement sleep wait in concurrent architecture
				smContext.SubPduSessLog.Infof(
					"PDUSessionSMContextUpdate, SMContext State[%v] should be SmStateInActivePending State",
					smContext.SMContextState.String(),
				)
			}
			// Send Release Notify to AMF
			smContext.SubPduSessLog.Infof(
				"PDUSessionSMContextUpdate, send Update SmContext Response",
			)
			...
			smContext.SubPduSessLog.Traceln(
				"PDUSessionSMContextUpdate, sent SMContext Status Notification successfully",
			)
		}
	} else {
...
func HandleUpCnxState(
	...
	case models.UpCnxState_ACTIVATING:
		smContext.SubPduSessLog.Infof(
			"PDUSessionSMContextUpdate, UP cnx state %v received",
			smContextUpdateData.UpCnxState,
		)
		if smContext.SMContextState != smf_context.SmStateActive {
			// Wait till the state becomes SmStateActive again
			// TODO: implement sleep wait in concurrent architecture
			smContext.SubPduSessLog.Infof(
				"PDUSessionSMContextUpdate, SMContext State[%v] should be SmStateActive State",
				smContext.SMContextState.String(),
			)
		}
		smContext.ChangeState(smf_context.SmStateModify)
		...
		response.JsonData.N2SmInfo = &models.RefToBinaryData{
			ContentId: "PDUSessionResourceSetupRequestTransfer",
		}
		...
		if err != nil {
			smContext.SubPduSessLog.Errorf(
				"PDUSessionSMContextUpdate, build PDUSession Resource Setup Request Transfer Error(%s)",
				err.Error(),
			)
		}
	...
	case models.UpCnxState_DEACTIVATED:
		...
		if smContext.SMContextState != smf_context.SmStateActive {
			...
			// TODO: implement sleep wait in concurrent architecture
			smContext.SubPduSessLog.Infof(
				"PDUSessionSMContextUpdate, SMContext State[%v] should be Active State",
				smContext.SMContextState.String(),
			)
		}
		if smContext.Tunnel != nil {
...
func HandleUpdateHoState(
	...
	case models.HoState_PREPARING:
		smContext.SubPduSessLog.Infof(
			"PDUSessionSMContextUpdate, Ho state %v received",
			smContextUpdateData.HoState,
		)
		...
		if smContext.SMContextState != smf_context.SmStateActive {
			...
			smContext.SubPduSessLog.Warnf(
				"PDUSessionSMContextUpdate, SMContext state[%v] should be SmStateActive",
				smContext.SMContextState.String(),
			)
		}
		smContext.ChangeState(smf_context.SmStateModify)
		...
		if err := smf_context.HandleHandoverRequiredTransfer(body.BinaryDataN2SmInformation, smContext); err != nil {
			smContext.SubPduSessLog.Errorf(
				"PDUSessionSMContextUpdate, handle HandoverRequiredTransfer failed: %+v",
				err,
			)
		}
		response.JsonData.N2SmInfoType = models.N2SmInfoType_PDU_RES_SETUP_REQ
	...
	case models.HoState_PREPARED:
		...
		if err := smf_context.HandleHandoverRequestAcknowledgeTransfer(body.BinaryDataN2SmInformation, smContext); err != nil { //nolint:lll
			smContext.SubPduSessLog.Errorf(
				"PDUSessionSMContextUpdate, handle HandoverRequestAcknowledgeTransfer failed: %+v",
				err,
			)
		}
...
func HandleUpdateCause(
	...
	case models.Cause_REL_DUE_TO_DUPLICATE_SESSION_ID:
		smContext.SubPduSessLog.Infof(
			"PDUSessionSMContextUpdate, update cause %v received",
			smContextUpdateData.Cause,
		)
		...
		smContext.SubCtxLog.Infof(
			"PDUSessionSMContextUpdate, Cause_REL_DUE_TO_DUPLICATE_SESSION_ID",
		)
...
func HandleUpdateN2Msg(
	...
	case models.N2SmInfoType_PDU_RES_SETUP_RSP:
		...
		if smContext.SMContextState != smf_context.SmStateActive {
			...
			// TODO: implement sleep wait in concurrent architecture
			smContext.SubPduSessLog.Warnf(
				"PDUSessionSMContextUpdate, SMContext state[%v] should be Active",
				smContext.SMContextState.String(),
			)
		}
		smContext.ChangeState(smf_context.SmStateModify)
		...
		if err := smf_context.
			...
			smContext.SubPduSessLog.Errorf(
				"PDUSessionSMContextUpdate, handle PDUSessionResourceSetupResponseTransfer failed: %+v",
				err,
			)
		}
	...
	case models.N2SmInfoType_PDU_RES_REL_RSP:
		...
		if smContext.PDUSessionRelease_DUE_TO_DUP_PDU_ID {
			if smContext.SMContextState != smf_context.SmStateInActivePending {
				...
				// TODO: implement sleep wait in concurrent architecture
				smContext.SubPduSessLog.Warnf(
					"PDUSessionSMContextUpdate, SMContext state[%v] should be ActivePending",
					smContext.SMContextState.String(),
				)
			}
			smContext.ChangeState(smf_context.SmStateInit)
			...
			problemDetails, err := consumer.SendSMContextStatusNotification(
				smContext.SmStatusNotifyUri,
			)
			if problemDetails != nil || err != nil {
				if problemDetails != nil {
					smContext.SubPduSessLog.Warnf(
						"PDUSessionSMContextUpdate, send SMContext Status Notification Problem[%+v]",
						problemDetails,
					)
				}
				...
				if err != nil {
					smContext.SubPduSessLog.Warnf(
						"PDUSessionSMContextUpdate, send SMContext Status Notification Error[%v]",
						err,
					)
				}
			} else {
			...
			smContext.SubCtxLog.Traceln("PDUSessionSMContextUpdate, SMContextState Change State: ", smContext.SMContextState.String()) //nolint:bodyclose, lll
		}
	case models.N2SmInfoType_PATH_SWITCH_REQ:
		...
		if err := smf_context.HandlePathSwitchRequestTransfer(body.BinaryDataN2SmInformation, smContext); err != nil {
			smContext.SubPduSessLog.Errorf(
				"PDUSessionSMContextUpdate, handle PathSwitchRequestTransfer: %+v",
				err,
			)
		}
		...
		if n2Buf, err := smf_context.BuildPathSwitchRequestAcknowledgeTransfer(smContext); err != nil {
			smContext.SubPduSessLog.Errorf(
				"PDUSessionSMContextUpdate, build Path Switch Transfer Error(%+v)",
				err,
			)
		} else {
			response.BinaryDataN2SmInformation = n2Buf
	...
	case models.N2SmInfoType_PATH_SWITCH_SETUP_FAIL:
		...
		if err := smf_context.HandlePathSwitchRequestSetupFailedTransfer(body.BinaryDataN2SmInformation, smContext); err != nil { //nolint:bodyclose, lll
